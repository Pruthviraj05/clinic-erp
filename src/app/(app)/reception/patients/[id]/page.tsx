...
export default async function ReceptionPatientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-4">
      <Link href="/reception/patients" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" /> Back to patients
      </Link>
      <PatientProfile bundle={bundle} qrDataUrl={qrDataUrl} canBill={can(user.role, "billing", "create")} />
    </div>
  );
}
